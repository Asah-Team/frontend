import React from "react";
import { Settings } from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, ResponsiveContainer } from "recharts";

export default function CardToolWear({ avgWear, progress, color }) {
  const chartData = [{ name: "Tool Wear", value: progress || 0 }];

  return (
    <div className="bg-white p-6 rounded-[20px] shadow border flex flex-col justify-between">
      {/* Header */}
      <div className="flex items-center gap-2">
        <Settings size={22} className="text-gray-600" style={{ color: color || "#0088FF" }}/>
        <p className="font-semibold text-gray-700">Rata-rata Tool Wear</p>
      </div>

      {/* Nilai utama */}
      <div className="mt-4">
        <h2 className="text-5xl font-bold" style={{ color: color || "#606060" }}>
          {avgWear !== null && avgWear !== undefined ? `${avgWear} mm` : "-"}
        </h2>
        <p className="text-sm text-gray-400 mt-1">Keausan Rata-rata</p>
      </div>

      {/* Progress Bar */}
      <div className="mt-4" style={{ height: "30px" }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} layout="vertical">
            <XAxis type="number" domain={[0, 100]} hide />
            <YAxis type="category" dataKey="name" hide />
            <Bar
              dataKey="value"
              barSize={20}
              fill={color}
              background={{ fill: "#e5e7eb", radius: 10 }}
              radius={[10, 10, 10, 10]}
            />
          </BarChart>
        </ResponsiveContainer>
      </div>

      <p className="text-sm text-gray-400 mt-2 text-right font-bold">
        {progress ? `${progress}%` : "-"}
      </p>
    </div>
  );
}